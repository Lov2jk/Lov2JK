...
const categoryDir=path.join(root,'content','categories');
const categoryFiles=fs.existsSync(categoryDir)?fs.readdirSync(categoryDir).filter(name=>name.endsWith('.json')):[];
const categories=categoryFiles.map(name=>JSON.parse(fs.readFileSync(path.join(categoryDir,name),'utf8')));
const categoryBySlug=new Map(categories.map(category=>[category.slug,category]));
const belongsTo=(slug,ancestor)=>{const seen=new Set();while(slug&&categoryBySlug.has(slug)&&!seen.has(slug)){if(slug===ancestor)return true;seen.add(slug);slug=categoryBySlug.get(slug).parent||''}return slug===ancestor};
...
write('visibility.json',{products:catalog.map(p=>({...identity(p),visible:p.visible!==false}))});
...
const requiredIssues=p=>{
  ...
  if((p.category==='Dresses'||belongsTo(p.category,'dresses'))&&!(p.sizes||[]).filter(Boolean).length)issues.push('Sizes');
